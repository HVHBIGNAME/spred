(() => {
  ...
  let kick = 0, loopFade = 0, hue = 330, targetHue = 330;
  ...
  function beat(power) { if (reduced) return; hook('onBeat', { power, hue, x: motion.x / 64, y: motion.y / 36 }); const x = motion.x * innerWidth / 64, y = Math.max(innerHeight * .45, motion.y * innerHeight / 36); if (rings.length < (innerWidth < 700 ? 2 : 4)) rings.push({ x, y, r: 9, a: .18, w: 1.2 + power }); for (let i = 0; i < 1 + Math.round(power * 2); i++) sparks.push({ x, y, vx: (Math.random() - .5) * 1.8, vy: -1 - Math.random() * 2, life: 1, size: Math.random() * 2 + 1 }); }
  function cutCheck(now) {
    ...
    l /= 2304; const maxBucket = Math.max(...buckets); if (maxBucket) targetHue = (buckets.indexOf(maxBucket) * 30 + 15) % 360; hue = (hue + angDiff(hue, targetHue) * .05 + 360) % 360; motion = sw ? { x: sx / sw, y: sy / sw, energy: clamp(sw / 230, 0, 1) } : { ...motion, energy: 0 }; const cut = lastLuma && Math.abs(l - lastLuma) > 32;
    if (cut) { hook('onCut', { x: motion.x / 64, y: motion.y / 36, hue }); const bandCol = hueCss(); flash.style.background = `radial-gradient(circle at ${motion.x / 64 * 100}% ${motion.y / 36 * 100}%, ${hueCss()} 0%, transparent 62%)`; flash.style.opacity = '.14'; band.style.background = bandCol; band.style.boxShadow = `0 0 18px 4px ${bandCol}`; band.classList.remove('run'); glitch.classList.remove('run'); void band.offsetWidth; band.classList.add('run'); glitch.classList.add('run'); setTimeout(() => { flash.style.opacity = '0'; glitch.classList.remove('run'); }, 300); }
  ...
  function draw(now) { const dt = Math.min(32, now - (draw.prev || now)); draw.prev = now; ctx.clearRect(0, 0, innerWidth, innerHeight); cutCheck(now); if (analyser) { analyser.getByteFrequencyData(data); analyser.getByteTimeDomainData(freq); let bass = 0; for (let i = 2; i < 14; i++) bass += data[i]; bass /= 12 * 255; energy = energy * .88 + bass * .12; threshold *= .985; if (bass > Math.max(.28, threshold + .08)) { threshold = bass; kick = 1; beat(clamp(bass, 0, 1)); } } kick *= .84; loopFade *= .88; const st = window.SpredCore.state; st.hue = hue; st.kick = kick; st.energy = energy; st.playing = running && !video.paused;
 root.style.setProperty('--acc', hueCss()); const rgb = hueRgb(); hook('onFrame', { hue, kick, energy }); const alpha = .05 + energy * .05; const grad = ctx.createRadialGradient(innerWidth * .72, innerHeight * .82, 0, innerWidth * .72, innerHeight * .82, innerWidth * .7); grad.addColorStop(0, `rgba(${rgb},${alpha})`); grad.addColorStop(1, `rgba(${rgb},0)`); ctx.fillStyle = grad; ctx.fillRect(0, 0, innerWidth, innerHeight); const grad2 = ctx.createRadialGradient(innerWidth * .45, innerHeight, 0, innerWidth * .45, innerHeight, innerWidth * .55); grad2.addColorStop(0, `rgba(${rgb},${alpha * .8})`); grad2.addColorStop(1, `rgba(${rgb},0)`); ctx.fillStyle = grad2; ctx.fillRect(0, 0, innerWidth, innerHeight);
    video.style.filter = `saturate(${1 + kick * .07})`; video.style.transform = `scale(${1.07 + kick * .008})`; video.style.opacity = String(1 - Math.min(1, loopFade * 7) * .55);
